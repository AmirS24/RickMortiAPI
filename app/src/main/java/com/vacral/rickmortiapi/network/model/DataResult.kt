package com.vacral.rickmortiapi.network.model
...
sealed interface DataResult<out D, out E : AppError>{
    ...
    fun errorOnNull(): E? = (this as? Error)?.error
}
...
inline fun <T,R,E : AppError> DataResult<T,E>.onSuccess(action: (T) -> Unit): DataResult<T,E>{
    if(this is DataResult.Success) action(data)
    return this
}

inline fun <T,R,E : AppError> DataResult<T,E>.onError(action: (E) -> Unit): DataResult<T,E>{
